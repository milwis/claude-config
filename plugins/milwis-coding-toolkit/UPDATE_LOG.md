...
### Updated
- **Agent frontmatter**: 7 agents moved from `model: opus` to `model: sonnet` — `php-pro`, `python-pro`, `javascript-pro`, `nextjs-pro`, `database-optimizer`, `test-automator`, `mobile-pwa-developer`. Five stay on `model: opus`: `code-reviewer`, `backend-security-coder`, `debugger`, `refactoring-orchestrator` (judgment gates) and `sql-pro` (irreversible output — a migration or DML statement against live data cannot be un-run, so there is no downstream gate that recovers the error).
