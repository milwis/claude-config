...
**Continuous queue** (`kolejka ciągła`) — the night queue without an end, for "fix bugs until there are none": started by the owner with `scripts/kolejka-ciagla/kolejka.sh start <repo>` (lead in tmux + watcher). Each cycle picks ONE open bug by priority P0 → P3 (`scripts/kolejka-ciagla/wybierz-issue.sh`; features, deferred and already-started issues are skipped), resolves it through one L2, merges it into local `main` and closes it; then the watcher types `/clear` and the start prompt, so every issue gets a fresh lead and the lead's window is never compacted. The repo runs no other lead meanwhile. Rules for the lead → `references/kolejka-ciagla.md`.
...
**A number from a SUBAGENT's report enters the ledger only after you re-run its command.** Every count, file list and zero a builder, reviewer, verifier or documentation subagent hands you is re-executed on your side — the same command, its raw output quoted next to the number — before it is written into a ledger row, an annex, an issue or the next brief; what you do not re-run is written as `reported, not re-measured`, never under `POMIAR`. The ledger is append-only and is read by every later generation, so a borrowed number is permanent. Two control commands cost ≈ 0 tokens and no spawn; the alternative is a false anchor the next builder pays to unlearn (§4, batch 2.5). `POMIAR` (batch 4.8): a documentation subagent returned three load-bearing sentences under `POMIAR`; the orchestrator's controls found one false (`git grep -c effective_date -- .claude CLAUDE.md coding-standards.md`: reported `0 trafień`, real value **2**) and one incomplete (`git grep -ln "#790" -- docs`: 3 files reported, **5** real). Its verdict ("documentation needs no change") was correct, and the disproved measurement **strengthened** it — the rule it was considering adding already existed at `.claude/agents/sql-pro.md:426`. A correct verdict is not evidence that the numbers under it are correct: it is the case in which a wrong number survives.

**A `0` is a measurement only after a second pattern or a positive control — for YOUR commands as well as theirs.** `git grep -E` does not know `\s` (POSIX `[[:space:]]`), `\b` and double-escaped ERE differ under BSD grep, and an unexpanded `$FILES` in zsh returns the same `0` as a clean tree. `POMIAR` (batch 4.8, intake): the generation's first corpus anchor was a false `0` in two of three trees from `\s*`; the same batch's second and third patterns gave the real 11 and revealed the out-of-scope two-argument variant that became the builder's main mine.
