...
2. Split findings by severity:
   ...
   - **Trivial fix-up exception (inline, no fixer spawn):** when a finding's fix is test-only, ≤ 40 lines, a copy of a neighbouring pattern already in the diff or the file, and provable by a one-command mutation probe — the orchestrator makes the edit itself (DIRECT, as in Step 0 Trivial) and quotes the probe's RED/GREEN line in the ledger/report. Anything outside those four bounds (production code, a new mechanism, no probe) goes to a fresh fixer as above. `POMIAR` (batch 2.4): three such fix-ups done inline cost ≈ 25k tokens; the same three through fresh fixer subagents would have cost ≥ 300k. Hard rule 6 still forbids resuming the builder — the choice is inline or fresh, never resumed.
