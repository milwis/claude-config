...
The **Model** column is the agent's own frontmatter default — you do not pass it. Every specialist in this toolkit runs on the Opus class, deliberately and uniformly (see the plugin README, "Model class"). The column is kept so the uniformity is visible rather than assumed: if a row ever reads anything else, that is a change someone made on purpose and the surrounding step should say why.
