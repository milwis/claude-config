...
- **Re-review of a fixer's commit → the fixer's NEW surface first.** Diff the fixer's commit(s) against the previous tip (the brief names it) and list every new guard, condition, branch or log line in production code; each one is matched against the fixer's mutant table, and a line without a RED row is a finding on its own (MEDIUM) — your first control mutant goes on such a line, on a copy. The previous findings tell you what the fixer was asked to prove; the untested surface is what it added while proving it. `POMIAR` (KonkretnyTMS batch 8.5, #486): R2's P2 (`checkBalance` guard tested only for `status=0`) and R3's P2 (`Circuit OPEN` log line, mutant GREEN) were both lines the fixer had just added — found last in each review, each one buying another ≈ 350k round.
...
- **DELETE** of a function/class/config key/cron daemon/route → run `scripts/check_orphan_tests.sh <symbol>` (or `grep -rn '<symbol>' tests/`). If hits remain → CRITICAL: tests reference a symbol that no longer exists. **A DELETE of a route, controller method, endpoint, permission entry or raw SQL call site additionally gets the project's counter latches run** (snapshot / budget / ratchet tests asserting an exact count of production artefacts — route registry, permission matrix, raw-PDO budget; the brief names them, otherwise `git grep -lE 'BUDGET|Snapshot' tests/`): they reference none of the diff's symbols, so the grep above is blind to them by construction, and a removal diff without the matching ratchet-down is a MEDIUM finding (`POMIAR` KonkretnyTMS batch 7.6, #408: an opus review passed a three-route removal; two of three counter latches went red only at the full suite). **A diff touching the SOURCE of a generated artefact** (route inventory, API listing, schema dump, permission-matrix document — anything a script regenerates and a freshness latch compares to its source) **without the regenerated artefact in the same diff is likewise MEDIUM** — run the project's freshness latches, or the generator, and cite the delta (`POMIAR` KonkretnyTMS batch 2.2, #802: 26 route edits passed an opus review; the inventory latch was the one red test of the end-of-batch suite).
...
A finding whose whole fix is comments or docblocks (no code token changes — a stale comment, a docblock over the cap, a misleading migration header) carries the tag **`comment-only`** in its first line, with the exact lines to change: the orchestrator applies such a finding inline under the task-lifecycle comment-only exception (`git diff -w --stat` + lint as proof), and a finding that mixes a comment cut with a code change is two findings, not one. `POMIAR` (KonkretnyTMS batch 2.2, #802): three comment-only P3s across `php/config`, `sql/migrations` and a test docblock went to a fixer spawn — 216k tokens for 26 comment lines; the same work inline costs ≈ 3k.
